 ...
 *
 */
public class RecallStepAction extends Action {

	private static final String actionName = "撤回步数打卡";

	public RecallStepAction() {
		super(actionName, RECALL_STEP_VAILD_KEYWORD_LIST);
		// TODO Auto-generated constructor stub
	}

	/**
	 * RecallStepAction类的合法参数及参数类型
	 */
	@SuppressWarnings("serial")
	private static final Map<String, MsgTypeEnum> RECALL_STEP_VAILD_KEYWORD_LIST = new HashMap<String, MsgTypeEnum>() {
		{
			put("撤回步数打卡", MsgTypeEnum.TEXT);
		}
	};

	protected Logger LOG = Logger.getLogger(RecallStepAction.class);
	...
	public String action(Group g, BaseMsg msg) {
		...
		if (group != null) {
			if (group.containsAction(WeeklyStepClockIn.class)) {
				...
				if (stepfile.exists()) {
					stepfile.delete();
					result = "@" + senderNickName + " 操作成功，你当天的步数打卡数据已经删除。";
				} else {
						...
						for (int i = array.length - 1; i >= 0; i--) {
							if (array[i].isFile()
								...
								result = "@" + senderNickName
										+ " 操作成功，你本周最近一次的步数打卡数据已经删除。";
								break;
							}
						}
						if (!isdelete) {
							result = "@" + senderNickName
									+ " 操作失败，你本周没有任何步数打卡数据，故无法删除。";
						}
					}
				}
			} else {
				result = "@"
						+ senderNickName
						+ " 本群不支持步数打卡功能，故无法撤回打卡数据。请联系管理员添加步数打卡的组件，或者移除撤回步数打卡的组件。";
			}
		}
		return result;
	}
