...
public class LoginServiceImpl implements ILoginService {
	...
	public String getUuid() {
		...
		params.add(new BasicNameValuePair(UUIDParaEnum.APP_ID.para(),
				UUIDParaEnum.APP_ID.value()));
		params.add(new BasicNameValuePair(UUIDParaEnum.FUN.para(),
				UUIDParaEnum.FUN.value()));
		params.add(new BasicNameValuePair(UUIDParaEnum.LANG.para(),
				UUIDParaEnum.LANG.value()));
		params.add(new BasicNameValuePair(UUIDParaEnum._.para(), String
				.valueOf(System.currentTimeMillis())));

		HttpEntity entity = httpClient.doGet(URLEnum.UUID_URL.getUrl(), params,
				true, null);
	...
	public boolean login() {
		...
		params.add(new BasicNameValuePair(LoginParaEnum.UUID.para(), core
				.getUuid()));
		params.add(new BasicNameValuePair(LoginParaEnum.TIP.para(),
				LoginParaEnum.TIP.value()));

		// long time = 4000;
		while (!isLogin) {
			// SleepUtils.sleep(time += 1000);
			long millis = System.currentTimeMillis();
			params.add(new BasicNameValuePair(LoginParaEnum.R.para(), String
					.valueOf(millis / 1579L)));
			params.add(new BasicNameValuePair(LoginParaEnum._.para(), String
					.valueOf(millis)));
			HttpEntity entity = httpClient.doGet(URLEnum.LOGIN_URL.getUrl(),
					params, true, null);
	...
	private void processLoginInfo(String loginContent) {
		...
		if (matcher.find()) {
			...
			Iterator<Entry<String, List<String>>> iterator = possibleUrlMap
					.entrySet().iterator();
			...
			while (iterator.hasNext()) {
				...
				fileUrl = "https://" + entry.getValue().get(0)
						+ "/cgi-bin/mmwebwx-bin";
				syncUrl = "https://" + entry.getValue().get(1)
						+ "/cgi-bin/mmwebwx-bin";
				if (core.getLoginInfo().get("url").toString()
						.contains(indexUrl)) {
			...
			core.getLoginInfo().put(
					"deviceid",
					"e"
							+ String.valueOf(new Random().nextLong())
									.substring(1, 16)); // 生成15位随机数
			...
			if (doc != null) {
				core.getLoginInfo().put(
						StorageLoginInfoEnum.skey.getKey(),
						doc.getElementsByTagName(
								StorageLoginInfoEnum.skey.getKey()).item(0)
								.getFirstChild().getNodeValue());
				core.getLoginInfo().put(
						StorageLoginInfoEnum.wxsid.getKey(),
						doc.getElementsByTagName(
								StorageLoginInfoEnum.wxsid.getKey()).item(0)
								.getFirstChild().getNodeValue());
				core.getLoginInfo().put(
						StorageLoginInfoEnum.wxuin.getKey(),
						doc.getElementsByTagName(
								StorageLoginInfoEnum.wxuin.getKey()).item(0)
								.getFirstChild().getNodeValue());
				core.getLoginInfo().put(
				...
				while (core.isAlive()) {
					try {
						...
						if (retcode.equals(RetCodeEnum.UNKOWN.getCode())) {
							...
							continue;
						} else if (retcode.equals(RetCodeEnum.LOGIN_OUT
								.getCode())) { // 退出
							LOG.info(RetCodeEnum.LOGIN_OUT.getType());
							break;
						} else if (retcode.equals(RetCodeEnum.LOGIN_OTHERWHERE
								.getCode())) { // 其它地方登陆
							LOG.info(RetCodeEnum.LOGIN_OTHERWHERE.getType());
							break;
						} else if (retcode.equals(RetCodeEnum.MOBILE_LOGIN_OUT
								.getCode())) { // 移动端退出
							LOG.info(RetCodeEnum.MOBILE_LOGIN_OUT.getType());
							break;
						} else if (retcode.equals(RetCodeEnum.NORMAL.getCode())) {
							core.setLastNormalRetcodeTime(System
									.currentTimeMillis()); // 最后收到正常报文时间
							...
							if (selector.equals("2")) {
								if (msgObj != null) {
									try {
										...
										for (int j = 0; j < msgList.size(); j++) {
											BaseMsg baseMsg = JSON
													.toJavaObject(msgList
															.getJSONObject(j),
															BaseMsg.class);
								...
								if (msgObj != null) {
									try {
										JSONArray modContactList = msgObj
												.getJSONArray("ModContactList");
										for (int j = 0; j < modContactList
												.size(); j++) {
											String groupUserName = modContactList
													.getJSONObject(j)
													.getString("UserName");
											String oldNickName = null;
											String groupNickName = modContactList
													.getJSONObject(j)
													.getString("NickName");
											 ...
											 * update grouplist
											 */
											List<JSONObject> groupList = Core
													.getInstance()
													.getGroupList();
											 ...
											 * update groupid
											 */
											List<String> groupIdList = Core
													.getInstance()
													.getGroupIdList();
											if (!groupIdList
													.contains(groupUserName)) {
												// LOG.info("已经添加进群ID列表:"
												// + groupUserName);
												Core.getInstance()
														.getGroupIdList()
												...
												// LOG.info("在群ID列表存在，无需重复添加:"
												// + groupUserName);
												/**
												 * update whitegrouplist
												 */
												for (int i = 0; i < Constant.groupList
														.size(); i++) {
													Group group = Constant.groupList
															.get(i);
													String groupId = group
															.getGroupId();
													if (groupId != null
															&& groupId
																	.equals(groupUserName)) {
														group.setGroupNickName(groupNickName);
														// LOG.info("群白名单列表里同Id的群已经更新:"
														// + group);
														break;
													}
												}
											...
											/**
											 * add groupNickNameList
											 */

											List<String> groupNickNameList = Core
													.getInstance()
													.getGroupIdList();
											if (isExist && oldNickName != null) {
												if (groupNickNameList
														.contains(oldNickName)) {
													groupNickNameList
															.remove(oldNickName);
													// LOG.info("已经从群NickName列表删除:"
													// + oldNickName);
												}
											}
											if (!groupNickNameList
													.contains(groupNickName)) {
												// LOG.info("已经添加进群NickName列表:"
												// + groupNickName);
												Core.getInstance()
														.getGroupIdList()
														.add(groupNickName);
								...
								if (msgObj != null) {
									try {
										...
										msgList = MsgCenter.produceMsg(msgList);
										for (int j = 0; j < msgList.size(); j++) {
											JSONObject userInfo = modContactList
													.getJSONObject(j);
											// 存在主动加好友之后的同步联系人到本地
	...
	private Map<String, String> syncCheck() {
		...
		// 组装请求URL和参数
		String url = core.getLoginInfo().get(
				StorageLoginInfoEnum.syncUrl.getKey())
		...
		params.add(new BasicNameValuePair("r", String.valueOf(new Date()
				.getTime())));
		params.add(new BasicNameValuePair("synckey", (String) core
				.getLoginInfo().get("synckey")));
		params.add(new BasicNameValuePair("_", String.valueOf(new Date()
				.getTime())));
		SleepUtils.sleep(7);
		try {
			...
			if (!matcher.find() || matcher.group(1).equals("2")) {
				LOG.info(String
						.format("Unexpected sync check result: %s", text));
			} else {
	...
	public boolean webWxInit() {
		...
		// 组装请求URL和参数
		String url = String.format(URLEnum.INIT_URL.getUrl(), core
				.getLoginInfo().get(StorageLoginInfoEnum.url.getKey()), String
				.valueOf(System.currentTimeMillis() / 3158L), core
		...
		try {
			...
			JSONObject user = obj.getJSONObject(StorageLoginInfoEnum.User
					.getKey());
			JSONObject syncKey = obj.getJSONObject(StorageLoginInfoEnum.SyncKey
					.getKey());

			core.getLoginInfo().put(
					StorageLoginInfoEnum.InviteStartCount.getKey(),
					obj.getInteger(StorageLoginInfoEnum.InviteStartCount
							.getKey()));
	...
	private JSONObject webWxSync() {
		...
		String url = String.format(URLEnum.WEB_WX_SYNC_URL.getUrl(), core
				.getLoginInfo().get(StorageLoginInfoEnum.url.getKey()), core
				.getLoginInfo().get(StorageLoginInfoEnum.wxsid.getKey()), core
				.getLoginInfo().get(StorageLoginInfoEnum.skey.getKey()), core
				.getLoginInfo().get(StorageLoginInfoEnum.pass_ticket.getKey()));
		Map<String, Object> paramMap = core.getParamMap();
		paramMap.put(StorageLoginInfoEnum.SyncKey.getKey(), core.getLoginInfo()
				.get(StorageLoginInfoEnum.SyncKey.getKey()));
		...
		try {
			...
			if (obj.getJSONObject("BaseResponse").getInteger("Ret") != 0) {
				...
				JSONArray syncArray = obj.getJSONObject(
						StorageLoginInfoEnum.SyncKey.getKey()).getJSONArray(
						"List");
				StringBuilder sb = new StringBuilder();
				for (int i = 0; i < syncArray.size(); i++) {
					sb.append(syncArray.getJSONObject(i).getString("Key") + "_"
